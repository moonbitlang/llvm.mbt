...
## 第五阶段：unsafe/Core.mbt 按功能族改造

`unsafe/Core.mbt` 在以下 commit 中会被多次修改。每个 commit 只能改表中列出的功能段；审核以函数族和对应 LLVM-C header contract 为单位，不以文件整体为单位。

### Commit 18：记录 Context、Module、Attribute 与 module metadata raw API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 `llvm_get_version` 到 Function 枚举/查询（当前约第 1—1629 行）：Context create/global/dispose 与 diagnostic；attribute kind/create/query；Module create/clone/dispose、identifier/source/data-layout/target/module-flag、print/dump/inline-asm；named metadata、debug location；Function add/find/first/last/next/previous |

- [ ] 为每组 allocator/disposer、borrowed string/message 和 global Context 写 Lifecycle/Safety。
- [ ] 区分底层 C-style 函数与 extension method 的等价关系，重复 wrapper 可简述但不能缺少文档。
- [ ] 对 deprecated alias、module mutation、inline assembly side effects 和 nullable lookup 明确说明。

建议提交信息：`docs(unsafe): document context and module bindings`

### Commit 19：记录 raw Type 构造与查询

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 `llvm_get_type_kind` 到 target extension type 参数查询（当前约第 1632—3065 行）：Type kind/context/print；integer/FP/function/struct/array/pointer/vector/abstract type 构造与查询；named/opaque struct body；target extension type query |

- [ ] 说明 global-context convenience 与 context-specific factory 的差异。
- [ ] 明确返回 TypeRef 的 Context ownership、struct body mutation、array/vector count 和 opaque pointer 语义。
- [ ] 对可能触发 LLVM assertion 的错误 kind、索引和跨 Context 输入写 Preconditions/Safety。

建议提交信息：`docs(unsafe): document type bindings`

### Commit 20：记录 raw Value inspection、ISA、use 与 operand API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 `LLVMValueRef::get_type` 到 `get_num_operands`（当前约第 3082—4449 行）：Value kind/name/print、RAUW、constant/undef/poison query；全部 `isa_*`；use-list 遍历；operand get/use/set/count；DbgRecord print |

- [ ] 说明 `isa_*` 返回的是 nullable casted handle，不是 Bool；说明 null sentinel 的检查方式。
- [ ] 对 RAUW、operand mutation、use iterator 和 erased value 的失效风险写 Lifecycle/Safety。
- [ ] 保留大量同族 ISA 方法的一句准确摘要，不增加机械 Example。

建议提交信息：`docs(unsafe): document value inspection bindings`

### Commit 21：记录 raw Constant 与 constant-expression API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 `LLVMTypeRef::const_null` 到 inline-asm constant（当前约第 4465—5692 行）：null/all-ones/undef/poison；integer/real/string/aggregate/vector constant；constant value extraction；constant expression arithmetic/GEP/cast/vector operations；block address与 constant inline asm |

- [ ] 说明字符串的 embedded NUL/length/UTF-8 边界、APInt 输入、signed/unsigned 读取和 exactness result。
- [ ] 明确 aggregate 元素的 Context/type 前置条件和 constant-expression 返回值生命周期。
- [ ] 对 deprecated constant-expression API 如实标注 LLVM 19 状态，不在文档任务中替换实现。

建议提交信息：`docs(unsafe): document constant bindings`

### Commit 22：记录 GlobalValue、Function、metadata 与 operand bundle raw API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 global parent/linkage 到 operand bundle（当前约第 5712—7410 行）：GlobalValue linkage/visibility/section/storage/alignment；global variable、alias、ifunc；Function personality/call-conv/GC/attributes/params；metadata string/node；operand bundle create/query/dispose |

- [ ] 明确 add/remove/erase、Module ownership transfer 或 detachment，以及 iterator/next handle 的失效条件。
- [ ] 说明 attribute index、string attribute、personality function、resolver 与 metadata operand 的类型约束。
- [ ] 为 operand bundle 明确 allocator/disposer 和 tag/argument borrowing。

建议提交信息：`docs(unsafe): document global and function bindings`

### Commit 23：记录 BasicBlock、Instruction、call/control-flow 与 Builder 状态

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 BasicBlock/value conversion 到 landing-pad cleanup（当前约第 7422—9919 行）：BasicBlock create/append/insert/delete/remove/move/traverse；Instruction metadata/parent/next/previous/remove/erase/delete/opcode/clone；DbgRecord；call-site/tail/callbr/invoke/successor/condition/switch/GEP/PHI/index API；Builder create/position/debug-location/insert/dispose；terminator、EH 与 landing-pad builder |

- [ ] 对 remove、erase、delete 和 clone 分别说明 parent 关系、ownership 与所有 alias 的失效风险。
- [ ] 明确 CFG mutation、successor/index、call-site-only 查询及 builder insertion point 的 Preconditions。
- [ ] 为 BuilderRef allocator/disposer 和 current debug location 写完整 Lifecycle。

建议提交信息：`docs(unsafe): document instruction and builder state bindings`

### Commit 24：记录 arithmetic、flag、memory 与 atomic property builder API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 `llvm_build_add` 到 atomic RMW opcode setter（当前约第 9997—11568 行）：整数/浮点 arithmetic、shift、bitwise、neg/not；NUW/NSW/exact/non-neg/fast-math/disjoint property；malloc/alloca/free、load/store/GEP/global string、memset/memcpy/memmove；volatile/weak/ordering/atomic-RMW property |

- [ ] 对同族 C-style/extension builder pair 使用一致描述，并准确记录 name 字符串和 CStringError。
- [ ] 说明 flags 是 semantic promise，错误设置可能产生 poison 或 invalid IR；不能写成纯优化开关。
- [ ] 明确 memory size/alignment/overlap、atomic ordering 和 instruction-kind 前置条件。

建议提交信息：`docs(unsafe): document arithmetic and memory builders`

### Commit 25：记录 cast、comparison、aggregate、atomic builder 与 legacy pass API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Core.mbt` | 从 `llvm_build_trunc` 到文件末尾（当前约第 11587—13437 行）：全部 cast；icmp/fcmp/phi/call/select/vaarg/vector/aggregate/freeze/null/ptrdiff builder；fence/atomic-rmw/cmpxchg/mask/ordering；MemoryBuffer create/query/dispose；legacy PassManager create/run/init/finalize/dispose；multithreading query |

- [ ] 说明 cast/source/destination、predicate、aggregate index、call type 与 atomic success/failure ordering 约束。
- [ ] 为 MemoryBuffer 的 copied/borrowed memory 与 disposer 写 Lifecycle。
- [ ] 为 legacy pass manager 的 Module/Function 模式、初始化顺序和 disposer 写契约，并标明 deprecated API。

建议提交信息：`docs(unsafe): document remaining core bindings`

## 检查点五：审核 unsafe/Core 分段结果

完成 Commit 18—25 后暂停，重点检查：

- 每个直接 binding 是否有准确 LLVM API 映射，extension wrapper 是否没有制造不同语义。
- nullability、owned message/string、borrowed handle、dispose 配对和 mutation 后失效是否覆盖完整。
- `LLVMBool` 中 LLVM 的“0 表示成功”是否在适用 API 上明确，不能统一按 MoonBit Bool 直觉解释。
- 文档是否基于 LLVM 19 header/行为；发现悬空 extern、错误返回模型或未实现 adapter 时是否停止，而不是把接口描述成可用。

## 第六阶段：其他 unsafe LLVM 子系统

### Commit 26：记录 Analysis、bitcode、IR reader、Comdat、Target 与 emission raw API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Analysis.mbt` | Module/Function verifier 与 CFG viewer |
| `unsafe/BitReader.mbt` | eager/lazy bitcode parse 的全部公开函数及 deprecated 入口 |
| `unsafe/BitWriter.mbt` | file/FD/memory-buffer bitcode writer |
| `unsafe/IRReader.mbt` | `llvm_parse_ir_in_context`、`LLVMContextRef::parse_ir` |
| `unsafe/Comdat.mbt` | Comdat get/set、selection kind query/mutation及 extension method |
| `unsafe/Remarks.mbt` | remark string data/length |
| `unsafe/Target.mbt` | `LLVMTargetDataRef`、native target/asm-printer 初始化、Module DataLayout、TargetData create 与 size/alignment query |
| `unsafe/TargetMachine.mbt` | Target 遍历/lookup/query；TargetMachineOptions create/set/dispose；TargetMachine create/dispose/configure/emit；host triple/CPU/features |

- [ ] 明确 verifier action/diagnostic、CFG viewer 外部副作用、bitcode/IR parser 对 MemoryBuffer ownership 的转移规则。
- [ ] 明确 file/FD 写入、FD close 选项和返回码约定。
- [ ] 为 Comdat、RemarkString 和 TargetData 的 borrowed/owned 生命周期写清 parent 或 disposer。
- [ ] 为 native 初始化的 process-global side effect、Target registry borrowing、TargetMachine/Options disposer 和 object emission message/file 契约写完整说明。

建议提交信息：`docs(unsafe): document parsing and target bindings`

### Commit 27：记录 Error、ExecutionEngine、LLJIT、ORC 与 Linker raw API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/Error.mbt` | consume/get-message/create-string-error 与 `LLVMErrorRef` 消费规则 |
| `unsafe/ExecutionEngine.mbt` | `LLVMExecutionEngineRef`、GenericValue create/query、engine/interpreter create、run、function address |
| `unsafe/LLJIT.mbt` | LLJIT/JITDylib/Builder ref type；builder/JIT create/dispose；main dylib、IR module add、lookup |
| `unsafe/Orc.mbt` | ThreadSafeContext/ThreadSafeModule ref type、create/dispose 与 Module 包装 |
| `unsafe/Linker.mbt` | `llvm_link_modules` |

- [ ] 把 `LLVMErrorRef` 的 consumed-on-read/consume-on-discard 规则写在每个相关入口上。
- [ ] 明确 ExecutionEngine 创建成功时接管 Module、GenericValue 与 engine disposer 当前是否绑定，以及运行函数的 ABI 风险。
- [ ] 说明 LLJIT/ORC owner graph、ThreadSafeModule 的 Module ownership transfer、symbol address 单位和 lookup error。
- [ ] 说明 linker 对 source Module 的 ownership transfer/销毁规则，不把 Bool 返回值写成普通成功布尔值。

建议提交信息：`docs(unsafe): document execution and ORC bindings`

### Commit 28：记录 DebugInfo、PassBuilder 与初始化 raw API

| 文件 | 公开对象 |
| --- | --- |
| `unsafe/DebugInfo.mbt` | 所有公开 DIBuilder create/finalize/dispose、DI file/type/location/variable/subprogram query、record insert、metadata replace、temporary node dispose 与 metadata-kind API |
| `unsafe/Transforms.mbt` | `llvm_run_passes`、PassBuilderOptions create/set/dispose 全部公开入口 |
| `unsafe/Initialization.mbt` | Core、TransformUtils、ScalarOpts、Vectorization、InstCombine、IPO、Analysis、IPA、CodeGen、Target pass registry 初始化函数 |

- [ ] 为 DIBuilder 的 Module borrowing、finalize-before-dispose 顺序、temporary metadata 与 inserted debug record 生命周期写完整契约。
- [ ] 说明 PassBuilder pipeline 字符串、TargetMachine nullable/borrowed 关系、LLVMErrorRef 返回值和每个 option 的实际含义。
- [ ] 说明 initialization 函数修改 process-global/pass-registry 状态及重复调用、线程同步要求。

建议提交信息：`docs(unsafe): document debug and pass bindings`

## 检查点六：审核其他 unsafe 子系统与最终覆盖

完成 Commit 26—28 后暂停，重点检查：

- parser、linker、ExecutionEngine、ORC 中的 Module/MemoryBuffer ownership transfer 是否逐项正确。
- ErrorRef、message、DIBuilder、TargetData、TargetMachine、PassBuilderOptions 是否都有准确 disposer 配对。
- 当前尚未接通或明显悬空的 extern 是否被识别为实现问题，而没有通过文档把它伪装为生产可用 API。
- `IR` 与 `unsafe` 两包的全部 active public declaration 是否完成逐项审计，`missing_doc` 是否归零。
